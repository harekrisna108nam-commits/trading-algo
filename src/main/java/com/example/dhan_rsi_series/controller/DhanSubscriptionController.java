...
public class DhanSubscriptionController {
    ...
    public Mono<ResponseEntity<String>> subscribe(@RequestBody SubscriptionRequest req) {
        ...
        return Flux.fromIterable(req.getSecurityIds())
            ...
            // 1️⃣ persist
            .doOnNext(securityId ->
                store.add(req.getExchange(), securityId,  req.getOptionType(), req.getMap().get(securityId).getStrike(), req.getMap().get(securityId).getExpiry())
            )
            ...
            .flatMap(securityId ->
                handler.subscribe(
                    ...
                    securityId,
                    req.getOptionType(),
                    req.getMap().get(securityId).getStrike(),
                    req.getMap().get(securityId).getExpiry()
                )
            )
