...
public class CandleRsiService {
	...
	/*
	 * private void save( String symbol, int securityId, String optionType, int
	 * timeframe, CandleSnapshot c, double rsi, int oi, int highestOi, double atp )
	 * {
	 * 
	 * OptionRsi e = new OptionRsi();
	 * 
	 * e.setSymbol(symbol); e.setSecurityId(securityId);
	 * e.setOptionType(optionType);
	 * 
	 * e.setTimeframe(toFrame(timeframe));
	 * 
	 * e.setCandleTime( Instant.ofEpochSecond(c.startEpoch())
	 * .atZone(ZoneId.of("Asia/Kolkata")) .toLocalDateTime() );
	 * 
	 * e.setOpen(c.open()); e.setHigh(c.high()); e.setLow(c.low());
	 * e.setClose(c.close());
	 * 
	 * e.setRsi(rsi);
	 * 
	 * // FIXED e.setOi(oi); e.setHighestOi(highestOi); e.setAtp(atp);
	 * 
	 * repo.save(e);
	 * 
	 * System.out.printf(
	 * "%s %s OHLC=[%.2f %.2f %.2f %.2f] RSI=%.2f OI=%d HighOI=%d ATP=%.2f%n",
	 * optionType, e.getTimeframe(), c.open(), c.high(), c.low(), c.close(), rsi,
	 * oi, highestOi, atp ); }
	 */
